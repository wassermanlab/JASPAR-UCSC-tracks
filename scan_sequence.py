...
def _scan_profile(profile, fasta_file, dummy_dir="/tmp/", output_dir="./",
    ...
    # Initialize
    pwmscan_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pwmscan")
    cutoff = None
    ...
    if not os.path.exists(output_file):
        ...
        cmd = "%s %s" % (os.path.join(pwmscan_dir, "matrix_prob"), pwm_file)
        ...
        # Scan FASTA file
        cmd_1 = "%s -m %s -c %s %s" % (os.path.join(pwmscan_dir, "matrix_scan"), pwm_file, cutoff, fasta_file)
